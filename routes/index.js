...
router.get("/cms", async function(req, res) {
  var res = new keystone.View(req, res);

  res.query("event", keystone.list("Event").model.find().sort("sortOrder"));

  const tac = await pg.query("SELECT * FROM tac");

  res.render("cms", {tac: tac.rows});
});
